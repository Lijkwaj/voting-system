...
exports.registerVoter = async function (userId, name, role) {
    ...
    try {
        ...
        const user = {
            affiliation: 'org1', enrollmentID: userId, role: 'client',
...
exports.createElection = async function (networkObj, electionCreatorId, electionId, electionName, electionParticipants) {
    try {
        let stringifiedParticipants = JSON.stringify(electionParticipants);
        console.log(electionId)
        let response = await networkObj.contract.submitTransaction('createElection', electionCreatorId, electionId, electionName, stringifiedParticipants);
        await networkObj.gateway.disconnect();
        return response.toString();
