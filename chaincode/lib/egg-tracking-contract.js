...
const EXTERNAL_API_SERVER = 'https://e7bb-86-88-151-70.ngrok-free.app';
...
const Election = require('./model/election.js');
...
class EggTrackingContract extends Contract {
  ...
  async createElection(ctx, electionCreatorId, electionId, electionName, electionParticipants) {
    ...
    const event = {
      ...
      targetAudience: [election.getOriginId()],
  ...
  isElectionCreator(identity) {
    return identity.assertAttributeValue('role', 'ElectionCreator');
  }
  ...
  isParticipant(identity) {
    return identity.assertAttributeValue('role', 'Participant');
  }
  ...
  async getFarmerStamp(id, timestamp) {
    try {
      ...
      const json = await axios(`${EXTERNAL_API_SERVER}/farmer/${id}?packingTimestamp=${timestamp}`);
